...
class ChatQUserListingAdapter(
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        if (item.last_online_ts == 0.0) {
            ...
            holder.ivOnlineIndicator.visibility = View.GONE
        } else {
            holder.tvLastSeen.text = item.lastOnlineTime(context)
            holder.ivOnlineIndicator.visibility = View.VISIBLE
        }

        itemView.setOnClickListener {
            onItemSelected(Actions.SELECT, item, position)
        }
    }
